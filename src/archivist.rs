...
use log::{info, warn};
use crate::{config::{stable, VolatileConfig}, ARPAError};
...
impl Archivist {
    ...
    pub async fn assert_unique<T>(
        &self, 
        item: &T,
    ) -> Result<()> where T: TableItem {
        let uniques = item.unique_values();
        if uniques.is_empty() { return Ok(()); }
        ...
        let id: Option<(i32,)> = sqlx::query_as(&query)
            .fetch_optional(&self.pool)
            .await?;

        id.map_or(
            Ok(()), 
            |(id,)| Err(ArchivistError::EntryAlreadyExists(
                item.insert_values(),
                T::TABLE.to_string(),
                id,
            ))
    ...
    pub async fn get_all<T>(
        &self, 
    ) -> Result<Vec<T>> where T: TableItem {
        let query = format!(
            "select {} from {};",
            T::select(),
            T::TABLE,
    ...
    pub async fn find<T>(
        &self, 
        condition: &str
    ) -> Result<Option<T>> where T: TableItem {
        let query = format!(
            "select {} from {} where {};",
            T::select(),
            T::TABLE,
    ...
    pub async fn delete<T>(&mut self, id: i32) -> Result<()> where T: TableItem {
    ...
    pub async fn get_special<U>(
        ...
        columns: &str,
        condition: &str,
    ) -> Result<Option<U>> where for<'r> U: FromRow<'r, PgRow> + Send + Unpin {
        let query = format!(
    ...
    pub(crate) const fn config(&self) -> &VolatileConfig {
        &self.config
    }
}
