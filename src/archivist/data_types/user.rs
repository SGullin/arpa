...
use crate::{archivist::table::TableItem, ARPAError, Result};
...
impl User {
    ...
    fn validate_username(name: &str) -> Result<String> {
        if name.len() > 12 || name.len() < 3 { 
            return Err(ARPAError::MalformedInput(
                format!("'{name}'; username must be 3--12 characters long."))
            );
        }
        if !name.is_ascii() {
            return Err(ARPAError::MalformedInput(
                format!("'{name}'; username must be only ASCII."))
            );
        }
        if name.contains(|c: char| c.is_ascii_whitespace()) {
            return Err(ARPAError::MalformedInput(
                format!("'{name}'; username cannot contain whitespace."))
            );
        }
    ...
    fn validate_name(name: &str) -> Result<String> {
        if name.len() < 3 { 
            return Err(ARPAError::MalformedInput(
                format!("'{name}'; name must be at over 2 characters long."))
            );
        }
    ...
    fn validate_email(email: &str) -> Result<String> {
        let at_pos = email
            .find('@')
            .ok_or(ARPAError::MalformedInput(
        ...
        email
            ...
            .filter(|p| *p > at_pos)
            .ok_or(ARPAError::MalformedInput(
                format!("'{email}'; Email addresses need a domain"), 
            ))?;
